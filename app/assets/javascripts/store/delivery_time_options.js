function SpreeDeliveryOptions() {
  ...
  this.initializeDatePicker = function() {
    $('#order_delivery_date').datepicker({dateFormat: "dd/mm/yy", minDate: 0});

    $('#order_delivery_date').on('change', function () {
      $.get('/checkout/delivery_slots?date=' + $(this).val(), function(response) {
        $('#order_delivery_slot_id option').remove();

        $.each(response, function(index, slot) {
          $('#order_delivery_slot_id').append('<option value="' + slot.id + '">' + slot.name + '</option>');
        });
      });
    });
  };
